...
def get_samples_by_indices(data_loader, indices):
    """
    从 data_loader 中提取出 indices 对应的样本
    """
    dataset = data_loader.dataset
    subset = Subset(dataset, indices)
    sub_loader1 = DataLoader(subset, batch_size=len(indices), shuffle=False, num_workers=data_loader.num_workers)
    
    all_indices = list(range(len(dataset)))
    remaining_indices = list(set(all_indices) - set(indices))
    remaining_subset = Subset(dataset, remaining_indices)
    sub_loader2 = DataLoader(remaining_subset, batch_size=data_loader.batch_size, shuffle=False, num_workers=data_loader.num_workers)
    return sub_loader1, sub_loader2
